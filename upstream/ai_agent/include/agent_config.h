...
/* A one-shot job that came due more than this long ago is dropped instead of
 * fired. This board boots to a seeded clock, so a reminder scheduled before
 * the clock synced can be days in the past the instant it does — firing those
 * would dump a pile of stale reminders on the user at once. Wide enough that
 * a genuinely due reminder still fires, including one that came due while the
 * board was briefly busy. */
#define AGENT_CRON_AT_STALE_GRACE (5 * 60)
#define AGENT_CRON_ID_LEN 9 /* 8 hex chars + NUL */
#define AGENT_CRON_STACK (8 * 1024)
#define AGENT_CRON_PRIO 40
/* One-shot boot task that syncs the clock from a network time source. Needs a
 * TLS-capable stack; priority sits just below the channels it must not delay. */
#define AGENT_CLOCK_SYNC_STACK (16 * 1024)
#define AGENT_CLOCK_SYNC_PRIO 45
