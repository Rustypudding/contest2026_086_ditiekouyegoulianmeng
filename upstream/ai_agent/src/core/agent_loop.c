...
static char* handle_nl_fast_path(const char* text)
{
     ...
     * Names only: skill_loader_build_summary() is written for the model and
     * carries a "read with: read_file <path>" hint on every line, which the
     * device then reads aloud to the kid verbatim. */
    static const char* kw_skill[] = {
        "技能列表", "有什么技能", "list skill", NULL };
    if (contains_any(text, kw_skill)) {
        char buf[2048];
        size_t n = skill_loader_list_names(buf, sizeof(buf));
        s_fast_path_count++;
...
static char* handle_slash_command(agent_msg_t* msg)
{
    ...
    if (strncmp(msg->content, "/help", 5) == 0) {
        ...
        /* Names only — the model-facing summary on every line ends with
         * "(read with: read_file /data/ai_agent/skills/<name>.md)", and the
         * device speaks whatever this returns. */
        char skills_buf[2048];
        size_t slen = skill_loader_list_names(
...
static char* handle_slash_remind(const agent_msg_t* msg)
{
    ...
    snprintf(input, sizeof(input),
        ...
        "\"channel\":\"%s\",\"chat_id\":\"%s\",\"relative\":true}",
    ...
    if (reply) {
        ...
        tool_registry_execute("cron_add", input, tool_out, sizeof(tool_out));

        /* cron_add can refuse, and replying "好的" regardless means the user
         * waits for a reminder that was never scheduled. */
        if (strncmp(tool_out, "Error", 5) == 0) {
            snprintf(reply, 512, "提醒没设上：%s", tool_out);
        } else {
    ...
    const char* channel, const char* chat_id)
{
    ...
    /* If context_build_system_prompt() filled the buffer to the brim, len can
     * reach size, and (size - len) then underflows size_t into a huge value —
     * snprintf would write past the calloc'd block. Leave the prompt as-is
     * rather than appending: a missing session block is survivable, a heap
     * overwrite is not. */
