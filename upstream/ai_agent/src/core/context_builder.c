...
/* Append a formatted chunk at off, never letting off leave [0, size - 1].
 *
 * snprintf returns the length it *would* have written, so the usual
 * "off += snprintf(buf + off, size - off, ...)" overshoots size the moment a
 * chunk gets truncated. Every later (size - off) then underflows size_t into
 * a huge value while buf + off is already past the end of the buffer —
 * snprintf sees "plenty of room" and writes an unbounded amount onto the
 * stack. The system prompt is assembled from a dozen such chunks and does get
 * truncated (the skills summary alone is ~1.7 KB), so every step is clamped
 * here instead. Returns the new offset; on truncation it stops at size - 1. */
...
static size_t build_tool_names(char *buf, size_t size)
{
    ...
    cJSON_ArrayForEach(item, arr) {
        ...
        if (name && cJSON_IsString(name)) {
            off = append_fmt(buf, size, off, "%s%s",
                           ...
                           const char *path, const char *header)
{
    ...
    /* A bare "fread(buf + offset, 1, size - offset - 1, f)" underflows that
     * length into a huge size_t once offset >= size, and fread then copies the
     * whole file straight through the end of the buffer. */
    if (offset < size - 1) {
...
int context_build_system_prompt(char *buf, size_t size)
{
    ...
    /* A clock that merely looks plausible is not one we can schedule
     * against: with no RTC the boot seed passes any magnitude check while
     * being days off. When it has not been synced, say so plainly and
     * forbid the model from computing an epoch off the header — that is
     * exactly how a reminder ends up on the wrong date. */
    bool clock_ok = agent_clock_is_verified();
    const char *time_note = clock_ok
        ? "For epoch, call get_current_time."
        : "UNVERIFIED — device clock NOT synced.";
    const char *time_rule = clock_ok
        ? "- Current time is in the header above. Answer time questions directly.\n"
        : "- Clock UNVERIFIED: the header time is NOT reliable. You MUST call "
          "get_current_time before any scheduling; never compute an epoch "
          "from the header.\n";

    off = append_fmt(buf, size, off,
        "# AI Agent\n\n"
        "Personal AI on Vela/NuttX. Channels: Feishu, WebSocket, CLI.\n"
        "Time: %s CST (UTC+8, tz=%s). %s\n\n"
        "## Rules\n"
        ...
        time_str, AGENT_TIMEZONE, time_note, time_rule);
    ...
    off = append_fmt(buf, size, off, "\n## Long-term Memory\n\n");
    {
        size_t avail = (off < size) ? size - off - 1 : 0;
    ...
    /* Skills summary. 2048 to match the other two skill_loader_build_summary()
     * call sites: the 11 built-in skills come to roughly 1.7 KB, which did not
     * fit the 1024 this used to be — the overflow leaked the loop's full_path
     * buffer into the prompt. This runs on the 32 KB agent thread and is the
