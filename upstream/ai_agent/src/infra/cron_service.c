...
static void cron_process_due_jobs(void)
{
    ...
    for (int i = 0; i < s_job_count; i++) {
        ...
        /* A one-shot can come due by a huge margin when the clock jumps
         * forward: this board boots to a seeded date, so anything scheduled
         * in that frame is hours-to-days past the moment the clock syncs.
         * Firing them would dump a pile of stale reminders on the user at
         * once, so drop them the way cron_service_start() already drops
         * stale jobs at boot. */
        if (job->kind == CRON_KIND_AT
            && (now - job->next_run) > AGENT_CRON_AT_STALE_GRACE) {
            syslog(LOG_WARNING,
                "[%s] Dropping stale one-shot '%s': due %lld, now %lld "
                "(> %d s late — clock jump or long power-off)\n",
                TAG, job->name, (long long)job->next_run, (long long)now,
                AGENT_CRON_AT_STALE_GRACE);
            job->enabled = false;
            job->next_run = 0;
            changed = true;
            continue;
