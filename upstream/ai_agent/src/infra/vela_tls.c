...
/* Clock nudging for certificate validation.
 *
 * This board has no RTC, so at boot the clock can still read 1970, and mbedTLS
 * then rejects every server certificate on its notBefore check. Below
 * CLOCK_TLS_MIN_EPOCH we stamp a PLACEHOLDER time that sits inside the
 * validity window so the handshake can proceed. It is chosen to satisfy
 * notBefore and nothing else — it is NOT a time source, and deliberately does
 * not mark the clock verified, so callers that need a trustworthy clock
 * (see agent_clock_is_verified()) still know it was never synced. */
#define CLOCK_TLS_MIN_EPOCH        ((time_t)1704067200) /* 2024-01-01 00:00 UTC */
#define CLOCK_TLS_PLACEHOLDER_EPOCH ((time_t)1772275200) /* 2026-02-28 00:00 UTC */
...
static int tls_ctx_connect(tls_ctx_t* ctx, const char* host, const char* port)
{
    ...
    if (now < CLOCK_TLS_MIN_EPOCH) {
        syslog(LOG_WARNING,
            "[%s] Clock too old (%ld) — forcing placeholder time for cert "
            "notBefore only; clock remains UNVERIFIED\n", TAG, (long)now);
        struct timespec ts = { .tv_sec = CLOCK_TLS_PLACEHOLDER_EPOCH, .tv_nsec = 0 };
        if (clock_settime(CLOCK_REALTIME, &ts) < 0) {
            syslog(LOG_ERR, "[%s] clock_settime failed: %d\n", TAG, errno);
        }
    }
