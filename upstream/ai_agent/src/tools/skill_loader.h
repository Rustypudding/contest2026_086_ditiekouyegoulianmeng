...
/**
 * List the available skill names only, comma-separated, for the *user*.
 *
 * skill_loader_build_summary() is written for the model: every entry carries
 * a description and a "read with: read_file <path>" hint. That text is not
 * meant to be spoken at a child, so anything the user sees or hears goes
