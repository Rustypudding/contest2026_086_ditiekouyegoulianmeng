    ...
    char* output, size_t output_size)
{
    ...
    /* A genuinely relative delay ("remind me in 30 minutes") is expressed as
     * now+delta and stays correct even on an unverified clock, so callers
     * that mean that set "relative": true. Everything else is an absolute
     * wall-clock time the model computed from a date it read, and that is
     * only meaningful if the clock itself can be trusted: this board has no
     * RTC, so it boots to a seed that is days off, and the job built from it
     * either fires early or is later dropped as stale without anyone being
     * told.
     *
     * Deliberately does NOT try to sync here: this runs inline on the
     * agent_loop thread and the TLS connect on this build has no usable
     * timeout, so a blocked time source would stall the whole turn. The
     * caller can recover by calling get_current_time, which does sync. */
    cJSON* rel_item = cJSON_GetObjectItem(root, "relative");
    bool relative = rel_item ? cJSON_IsTrue(rel_item) : false;

    if (!relative && !agent_clock_is_verified()) {
        snprintf(output, output_size,
            "Error: device clock is NOT synced, so an absolute time cannot be "
            "scheduled yet. Call get_current_time first — it syncs the clock — "
            "then retry this call with the corrected at_epoch. If "
            "get_current_time also fails, the network time source is "
            "unreachable: tell the user the device clock is uncalibrated and "
            "that the reminder was NOT set.");
        return ERROR;
    }
